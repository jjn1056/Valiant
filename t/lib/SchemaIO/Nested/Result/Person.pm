package SchemaIO::Nested::Result::Person;
...
__PACKAGE__->belongs_to(
  state =>
  'SchemaIO::Nested::Result::State',
  { 'foreign.id' => 'self.state_id' }
);

__PACKAGE__->has_many(
  person_roles =>
  'SchemaIO::Nested::Result::PersonRole',
  { 'foreign.person_id' => 'self.id' }
);

__PACKAGE__->has_many(
  meetings =>
  'SchemaIO::Nested::Result::Meeting',
  { 'foreign.person_id' => 'self.id' }
);
...
__PACKAGE__->accept_nested_for('person_roles', +{find_with_uniques=>'allow_create', allow_destroy=>1});
__PACKAGE__->accept_nested_for('roles', +{find_with_uniques=>'allow_create', allow_destroy=>1});

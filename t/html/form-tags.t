...

{
  ...
  has ['name', 'roles'] => (is=>'ro', required=>1);

  1;
}

{
  ok my $user = Local::Role->new({id=>100, label=>'user'});

  is field_id($user, 'label'), 'local_role_label';
  is field_id($user, 'label', 2), 'local_role_2_label';
  is field_id($user, 'label', 2, 'foo'), 'foo_local_role_2_label';

  is field_name($user, 'label'), 'role.label';
  is field_name($user, 'label', [qw/foo bar/]), 'role.label.foo.bar';
  is field_name($user, 'label', +{index=>10}), 'role[10].label';
  is field_name($user, 'label', +{index=>10, multiple=>1}), 'role[10].label[]';
  is field_name($user, 'label', [qw/baz foo/], +{index=>10, multiple=>1}), 'role[10].label.baz.foo[]';
}

{
  ...
  my $roles = Valiant::HTML::Util::Collection->new($user, $admin, $guest);

  warn collection_select($user, 'id', $roles, 'id', 'label', {class=>'foo'});

...
use DBIO::Valiant::Util::Exception::TooManyRows;
use DBIO::Valiant::Util::Exception::BadParameterFK;
use DBIO::Valiant::Util::Exception::BadParameters;
use Valiant::I18N;
...
sub update {
  ...
  foreach my $related(keys %related) {
    ...
    if($has_limit) {
      ...
      DBIO::Valiant::Util::Exception::TooManyRows
        ->throw(
          limit=>$limit,
          attempted=>$num,
          related=>$related,
      ...
      foreach my $fk_proto (keys %cond) {
        ...
        if(exists $param_row->{$fk}) {
          unless($param_row->{$fk} eq $self->get_column($pk)) {
            DBIO::Valiant::Util::Exception::BadParameterFK->throw(
  ...
  if($related_result) {
    ...
    if(blessed($params)) {
      ...
      if($self->in_storage) {
        ...
        if($flag == scalar(@pks)) {
          DBIO::Valiant::Util::Exception::BadParameters->throw(related=>$related, me=>$self);
        }
...
DBIO::Valiant::Result - Base component to add Valiant functionality

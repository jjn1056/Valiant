package DBIO::Valiant::ResultSet;
...
sub set_recursively {
  ...
  foreach my $row_data (@$data) {
    ...
    } elsif (ref($row_data) && $row_data->isa('DBIO::Row')) {
...
sub _dump_row {
  ...
  foreach my $rel_name (@relationships) {
    ...
    if (defined $related) {
      if ($related->isa('DBIO::ResultSet')) {
        $data{$rel_name} = $related->_dump_resultset($seen);
      } elsif ($related->isa('DBIO::Row')) {
...
DBIO::Valiant::ResultSet - Validation support for resultsets
...
classes if you plan to use L<DBIO::Valiant> across all your result classes.
...
L<Valiant>, L<DBIO>, L<DBIO::Valiant>

...
DBIO::Valiant - Glue Valiant validations into DBIO

=head1 SYNOPSIS

You need to add the components L<DBIO::Valiant::Result> and L<DBIO::Valiant::ResultSet>
...
classes glue L<Valiant> into L<DBIO>.  You can set filters and validations on
...
This has L<DBIO::Candy> integration if you use that and prefer it:
    ...
    use DBIO::Candy -base => 'Schema::Result';
...
When using this with L<DBIO::Candy> the following class methods are available as exports:
...
Assuming you have a base result class C<Example::Schema::Result> which uses the L<DBIO::Valiant::Result>
...
Here's how we setup these related classes.  Unlike the older DBIC-based code, DBIO records
C<many_to_many> metadata natively, and L<DBIO::Valiant::Result> uses that native registry
directly rather than maintaining its own shadow copy (see L</Many to Many> below).
...
Unlike DBIC, DBIO records C<many_to_many> metadata natively (via C<_m2m_metadata> on your
result class), so L<DBIO::Valiant::Result> uses that native registry directly instead of
maintaining its own shadow copy.  Many to many relationships are validated and nested-created the
same way as any other relationship described in this document.
...
L<Valiant>, L<DBIO::Valiant::Result>, L<DBIO::Valiant::ResultSet>, L<DBIO>

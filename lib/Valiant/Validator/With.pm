...
sub validate_each {
  ...
  if($self->has_cb) {
    my $return = $self->cb->($record, $attribute, $value, $self->options);
    $record->errors->add($attribute, $self->message_if_false, $self->options)
      if !$return and $self->has_message_if_false;
  }
  if($self->has_method) {
    if(my $method_cb = $record->can($self->method)) {
      my $return = $method_cb->($record, $attribute, $value, $self->options);
      $record->errors->add($attribute, $self->message_if_false, $self->options)
        if !$return and $self->has_message_if_false;
    } else {
